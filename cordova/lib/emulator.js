...
module.exports.install = function (givenTarget, buildResults) {
    ...
    // install the app
        ...
        return Q.when().then(function () {
            ...
            events.emit('log', 'Package name: ' + "not support");
            ...
            function installPromise () {
                return adbInstallWithOptions(target.target, apk_path, execOptions).catch(function (error) {
                    ...
                    // This promise is always resolved, even if 'adb uninstall' fails to uninstall app
                    // or the app doesn't installed at all, so no error catching needed.
                    return Adb.uninstall(target.target, "not support").then(function () {
    ...
    // unlock screen
        ...
        Adb.start(target.target, "not support" + '/.' + manifest.getActivity().getName());
